...
class ConformerNaiveEncoder(nn.Module):
    ...
    def setup(self):
        self.encoder_layers =  [
                CFNEncoderLayer(self.dim_model, self.num_heads, self.use_norm, self.conv_only, self.conv_dropout, self.atten_dropout)
                for _ in range(self.num_layers)
            ]
        

    def __call__(self, x, mask=None,deterministic=None) -> jnp.ndarray:
        """
        Args:
...
class CFNEncoderLayer(nn.Module):
    ...
    def setup(self):
        ...
        #if not conv_only:
        #  self.attn = SelfAttention(dim=self.dim_model,
    ...
    def __call__(self, x, mask=None,deterministic=None) -> jnp.ndarray:
        ...
        x = x + (self.conformer(x,deterministic=deterministic))
...
class ConformerConvModule(nn.Module):
    ...
    def setup(self):
        ...
        self.net = nn.Sequential([
            ...
            nn.glu,
            nn.Conv(inner_dim, kernel_size=self.kernel_size, feature_group_count=inner_dim),
            #DepthWiseConv1d(inner_dim, inner_dim, kernel_size=kernel_size, groups=inner_dim),
            nn.silu,
            nn.Conv(self.dim, 1),
            ]
        )
...
#     groups:int
#     def setup(self):
#         self.conv = nn.Conv(chan_out, kernel_size=kernel_size, feature_group_count=groups)
...
class Transpose(nn.Module):
    ...
    def setup(self):
        #super().__init__()
        assert len(self.dims) == 2, 'dims must be a tuple of two dimensions'

    def __call__(self, x):
        return jnp.swapaxes(x,*self.dims)
